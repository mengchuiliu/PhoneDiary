...
public class CalendarCard extends View {
    ...
    public CalendarCard(Context context, OnCellClickListener listener, String chooseDate) {
        super(context);
    ...
    private void fillDate() {
        ...
        for (int j = 0; j < TOTAL_ROW; j++) {
            ...
            for (int i = 0; i < TOTAL_COL; i++) {
                ...
                if (rows[j].cells[i].date.toString().equals(chooseDate)) {
                    rows[j].cells[i].state = State.CHO0SE;
                }
            }
        }
...
//        mCellSpace = Math.min(h / TOTAL_ROW, w / TOTAL_COL);
    ...
    private void measureClickCell(int col, int row) {
        ...
        if (rows[row] != null) {
            ...
            rows[row].cells[col].state = State.CLICKED;

            CustomDate date = rows[row].cells[col].date;
            date.week = col;
            Log.e("my_log", "===点击返回日期===>" + date.toString());
            mCellClickListener.clickDate(date);
            // 刷新界面
            if (DateUtil.getDateLong(date.toString()) < System.currentTimeMillis()) {
                invalidate();
            }
        }
    }
...
//                    mCirclePaint.setStrokeWidth(3);
                    canvas.drawCircle((float) (mCellSpace * (i + 0.5)), (float) ((j + 0.5) * mCellSpace),
                ...
                case PAST_MONTH_DAY: // 过去一个月
                    mTextPaint.setColor(Color.parseColor("#e600e6"));
                case NEXT_MONTH_DAY: // 下一个月
                    mTextPaint.setColor(Color.parseColor("#868686"));
                    break;
...
//                    mCirclePaint.setColor(Color.parseColor("#66D7B9")); // 绿色圆形
...
//                    mCirclePaint.setColor(Color.parseColor("#F24949")); // 红色圆形
     ...
     */
    enum State {
        TODAY, CURRENT_MONTH_DAY, PAST_MONTH_DAY, NEXT_MONTH_DAY, UNREACH_DAY, CLICKED, CHO0SE
    }
